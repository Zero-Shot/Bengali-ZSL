...
def get_number_of_words(txt_file):
    with io.open(txt_file, encoding="utf-8") as file:
        return sum(1 for _ in file)


def get_number_of_columns(alphabet_csv):
    with io.open(alphabet_csv, encoding="utf-8") as file:
...
def run(alphabet_csv, word_txt, s_matrix_csv, out_dir="out", prefix=""):
    if os.path.isdir(word_txt):
        ...
        for file in os.listdir(word_txt):
            ...
            if os.path.isfile(file_path) and file.endswith(".txt"):
                create_s(alphabet_csv, file_path, os.path.join(outpath, prefix + "S4matrix_" + os.path.basename(file)))
    else:
...
if __name__ == '__main__':
    ...
    run(alphabet_csv, word_txt, s_matrix_csv)
